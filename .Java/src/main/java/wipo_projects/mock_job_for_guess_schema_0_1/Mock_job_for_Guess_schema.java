	...
	public void tJavaFlex_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String url = "jdbc:jtds:sqlserver://localhost:1433/MARCAS_PY;";
...
// Exception is thrown if db don't support, no need to catch exception here
				...
				java.sql.ResultSet rs = stm.executeQuery(
						"SELECT DISTINCT  				 1 row_version,  				 cast(reception_date as DATE) ,  				 CAST(DATEPART(YYYY, reception_date) as NUMERIC),  				 case  				    when datepart(dw, cast(reception_date as DATE)) = 7 /* Saturday */  				            then dateadd(dd, 2, cast(reception_date as DATE))  				    when datepart(dw, cast(reception_date as DATE)) = 1 /* Sunday */  				            then dateadd(dd, 1, cast(reception_date as DATE))  				    else null  				 end  		FROM  wrk_mark   		WHERE	 cast(reception_date as DATE)  not in  			(  		 	SELECT daily_date  			FROM  IP_DAY   			) order by 3");
				java.sql.ResultSetMetaData rsmd = rs.getMetaData();
				int numbOfColumn = rsmd.getColumnCount();

				String fileName = (new java.io.File(
						"G:/TOS_DI-20201015_1726-V7.4.1M3/temp/TempGuessSchemaDelimitedFile.csv")).getAbsolutePath()
								.replace("\\", "/");
...
/************************************************************************************************
 * 54549 characters generated by Talend Open Studio for Data Integration on the
 * November 5, 2020 at 4:33:50 PM CET
 ************************************************************************************************/