...
package wipo_projects.mark_convert_0_3;
 ...
 * @status
 */
public class MARK_CONVERT implements TalendJob {
	...
	private final String jobName = "MARK_CONVERT";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(MARK_CONVERT.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public void tOracleInput_2_error(Exception exception, String errorComponent,
			final java.util.Map<String, Object> globalMap) throws TalendException {
	...
	public void tOracleOutput_2_error(Exception exception, String errorComponent,
			final java.util.Map<String, Object> globalMap) throws TalendException {
	...
	public static class sectordataStruct implements routines.system.IPersistableRow<sectordataStruct> {
		final static byte[] commonByteArrayLock_WIPO_PROJECTS_MARK_CONVERT = new byte[0];
		static byte[] commonByteArray_WIPO_PROJECTS_MARK_CONVERT = new byte[0];

		public BigDecimal SECTOR_NBR;

		public BigDecimal getSECTOR_NBR() {
			return this.SECTOR_NBR;
		}

		public String SECTOR_DATA;

		public String getSECTOR_DATA() {
			return this.SECTOR_DATA;
		}

		public BigDecimal IMAGE_FORMAT_WCODE;

		public BigDecimal getIMAGE_FORMAT_WCODE() {
			return this.IMAGE_FORMAT_WCODE;
		}

		public String IND_BASE64;

		public String getIND_BASE64() {
			return this.IND_BASE64;
		}
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_WIPO_PROJECTS_MARK_CONVERT.length) {
					if (length < 1024 && commonByteArray_WIPO_PROJECTS_MARK_CONVERT.length == 0) {
						commonByteArray_WIPO_PROJECTS_MARK_CONVERT = new byte[1024];
					} else {
						commonByteArray_WIPO_PROJECTS_MARK_CONVERT = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_WIPO_PROJECTS_MARK_CONVERT, 0, length);
				strReturn = new String(commonByteArray_WIPO_PROJECTS_MARK_CONVERT, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_WIPO_PROJECTS_MARK_CONVERT) {

				try {

					int length = 0;

					this.SECTOR_NBR = (BigDecimal) dis.readObject();

					this.SECTOR_DATA = readString(dis);

					this.IMAGE_FORMAT_WCODE = (BigDecimal) dis.readObject();

					this.IND_BASE64 = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeObject(this.SECTOR_NBR);

				// String

				writeString(this.SECTOR_DATA, dos);

				// BigDecimal

				dos.writeObject(this.IMAGE_FORMAT_WCODE);

				// String

				writeString(this.IND_BASE64, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("SECTOR_NBR=" + String.valueOf(SECTOR_NBR));
			sb.append(",SECTOR_DATA=" + SECTOR_DATA);
			sb.append(",IMAGE_FORMAT_WCODE=" + String.valueOf(IMAGE_FORMAT_WCODE));
			sb.append(",IND_BASE64=" + IND_BASE64);
			sb.append("]");
		 ...
		 * Compare keys
		 */
		public int compareTo(sectordataStruct other) {
	...
	public static class currentMarkStruct implements routines.system.IPersistableRow<currentMarkStruct> {
		final static byte[] commonByteArrayLock_WIPO_PROJECTS_MARK_CONVERT = new byte[0];
		static byte[] commonByteArray_WIPO_PROJECTS_MARK_CONVERT = new byte[0];
		 ...
		 * Compare keys
		 */
		public int compareTo(currentMarkStruct other) {
	...
	public void tOracleInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				currentMarkStruct currentMark = new currentMarkStruct();
				sectordataStruct sectordata = new sectordataStruct();

				/**
				 * [tFlowToIterate_1 begin ] start
				 */

				int NB_ITERATE_tOracleInput_2 = 0; // for statistics
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "currentMark");
				}
				...
				String dbquery_tOracleInput_1 = "select  distinct FILE_SEQ, FILE_TYP, FILE_SER, FILE_NBR from IP_LOGO_SECTIONS\nwhere FILE_SEQ IN ("
				...
				try {
					...
					while (rs_tOracleInput_1.next()) {
						...
						System.out.println("Converting: " + row1.FILE_SEQ + "/" + row1.FILE_TYP + "/" + row1.FILE_SER
								+ "/" + row1.FILE_NBR);
						currentMark.FILE_SEQ = row1.FILE_SEQ;
						currentMark.FILE_TYP = row1.FILE_TYP;
						currentMark.FILE_SER = row1.FILE_SER;
						currentMark.FILE_NBR = row1.FILE_NBR;
						...
						globalMap.put("currentMark.FILE_SEQ", currentMark.FILE_SEQ);

						globalMap.put("currentMark.FILE_TYP", currentMark.FILE_TYP);

						globalMap.put("currentMark.FILE_SER", currentMark.FILE_SER);

						globalMap.put("currentMark.FILE_NBR", currentMark.FILE_NBR);
						...
						if (execStat) {
							runStat.updateStatOnConnection("rowDelete", 3, 0);
						}

						if (execStat) {
							runStat.updateStatOnConnection("sectordata", 3, 0);
						}
						...
						if (execStat) {
							runStat.updateStatOnConnection("iterate1", 1, "exec" + NB_ITERATE_tOracleInput_2);
							// Thread.sleep(1000);
						}
						...
						if (execStat) {
							runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "sectordata");
						}
						...
						String dbquery_tOracleInput_2 = "\nSELECT  ls.SECTOR_NBR,\n        ls.SECTOR_DATA,\n        l.IMAGE_FORMAT_WCODE,\n        l.IND_BASE64\nFROM IP_LOGO_SE"
								+ "CTIONS ls\nINNER JOIN IP_LOGO l\nON ls.FILE_SEQ = l.FILE_SEQ and\nls.FILE_TYP = l.FILE_TYP and\nls.FILE_SER = l.FILE_SER"
								+ " and\nls.FILE_NBR = l.FILE_NBR\nWHERE " + " ls.FILE_SEQ = '"
								+ globalMap.get("currentMark.FILE_SEQ").toString() + "' and " + " ls.FILE_TYP = '"
								+ globalMap.get("currentMark.FILE_TYP").toString() + "' and " + " ls.FILE_SER = "
								+ globalMap.get("currentMark.FILE_SER").toString() + " and " + " ls.FILE_NBR = "
								+ globalMap.get("currentMark.FILE_NBR").toString() + " ORDER BY ls.SECTOR_NBR";
						...
						try {
							...
							while (rs_tOracleInput_2.next()) {
								...
								if (colQtyInRs_tOracleInput_2 < 1) {
									sectordata.SECTOR_NBR = null;
								} else {

									if (rs_tOracleInput_2.getObject(1) != null) {
										sectordata.SECTOR_NBR = rs_tOracleInput_2.getBigDecimal(1);
									} else {
								...
								if (colQtyInRs_tOracleInput_2 < 2) {
									sectordata.SECTOR_DATA = null;
								} else {

									sectordata.SECTOR_DATA = routines.system.JDBCUtil.getString(rs_tOracleInput_2, 2,
											false);
								}
								if (colQtyInRs_tOracleInput_2 < 3) {
									sectordata.IMAGE_FORMAT_WCODE = null;
								} else {

									if (rs_tOracleInput_2.getObject(3) != null) {
										sectordata.IMAGE_FORMAT_WCODE = rs_tOracleInput_2.getBigDecimal(3);
									} else {
								...
								if (colQtyInRs_tOracleInput_2 < 4) {
									sectordata.IND_BASE64 = null;
								} else {

									sectordata.IND_BASE64 = routines.system.JDBCUtil.getString(rs_tOracleInput_2, 4,
											false);
								}
								...
								base64image.append(sectordata.SECTOR_DATA.toString());
						...
						globalMap.put("imagedata", IPAS.base64toByteArray(base64image.toString()));

						if (execStat) {
							runStat.updateStat(resourceMap, iterateId, 2, 0, "sectordata");
						}
						...
						if (execStat) {
							runStat.updateStatOnConnection("iterate1", 2, "exec" + NB_ITERATE_tOracleInput_2);
						}
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "currentMark");
				}
	...
	public static class rowDeleteStruct implements routines.system.IPersistableRow<rowDeleteStruct> {
		final static byte[] commonByteArrayLock_WIPO_PROJECTS_MARK_CONVERT = new byte[0];
		static byte[] commonByteArray_WIPO_PROJECTS_MARK_CONVERT = new byte[0];
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final rowDeleteStruct other = (rowDeleteStruct) obj;
		...
		public void copyDataTo(rowDeleteStruct other) {
		...
		public void copyKeysDataTo(rowDeleteStruct other) {
		 ...
		 * Compare keys
		 */
		public int compareTo(rowDeleteStruct other) {
	...
	public void tFixedFlowInput_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				rowDeleteStruct rowDelete = new rowDeleteStruct();
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "rowDelete");
				}

				int tos_count_tOracleOutput_2 = 0;

				int deleteKeyCount_tOracleOutput_2 = 4;
				if (deleteKeyCount_tOracleOutput_2 < 1) {
				...
				if (dbschema_tOracleOutput_2 == null || dbschema_tOracleOutput_2.trim().length() == 0) {
					tableName_tOracleOutput_2 = ("IP_LOGO_SECTIONS");
				} else {
					tableName_tOracleOutput_2 = dbschema_tOracleOutput_2 + "." + ("IP_LOGO_SECTIONS");
				}
				String delete_tOracleOutput_2 = "DELETE FROM " + tableName_tOracleOutput_2
				...
				java.sql.PreparedStatement pstmt_tOracleOutput_2 = conn_tOracleOutput_2
						.prepareStatement(delete_tOracleOutput_2);
...
// # Outputs initialization
				rowDeleteStruct rowDelete_tmp = new rowDeleteStruct();
// ###############################
				...
				String update_tOracleOutput_1 = "UPDATE " + tableName_tOracleOutput_1
						+ " SET LOGO_DATA = ?,IND_BASE64 = ? WHERE FILE_SEQ = ? AND FILE_TYP = ? AND FILE_SER = ? AND FILE_NBR = ?";
...
// ###############################
				...
				for (int i_tFixedFlowInput_1 = 0; i_tFixedFlowInput_1 < 1; i_tFixedFlowInput_1++) {
					...
					row2.FILE_SEQ = (String) globalMap.get("currentMark.FILE_SEQ");

					row2.FILE_TYP = (String) globalMap.get("currentMark.FILE_TYP");

					row2.FILE_SER = (BigDecimal) globalMap.get("currentMark.FILE_SER");

					row2.FILE_NBR = (BigDecimal) globalMap.get("currentMark.FILE_NBR");

					row2.LOGO_DATA = (byte[]) globalMap.get("imagedata");
...
// Start of branch "update"
					if (update != null) {
						...
						if (update.IND_BASE64 == null) {
							pstmt_tOracleOutput_1.setNull(2, java.sql.Types.VARCHAR);
						} else {
							pstmt_tOracleOutput_1.setString(2, update.IND_BASE64);
						}
						...
						if (!whetherReject_tOracleOutput_1) {
							row3 = new row3Struct();
							row3.FILE_SEQ = update.FILE_SEQ;
							row3.FILE_TYP = update.FILE_TYP;
							row3.FILE_SER = update.FILE_SER;
							row3.FILE_NBR = update.FILE_NBR;
							row3.LOGO_DATA = update.LOGO_DATA;
							row3.IND_BASE64 = update.IND_BASE64;
						}
...
// # Output table : 'rowDelete'
								rowDelete_tmp.FILE_SEQ = row3.FILE_SEQ;
								rowDelete_tmp.FILE_TYP = row3.FILE_TYP;
								rowDelete_tmp.FILE_SER = row3.FILE_SER;
								rowDelete_tmp.FILE_NBR = row3.FILE_NBR;
								rowDelete = rowDelete_tmp;
// ###############################
...
// Start of branch "rowDelete"
							if (rowDelete != null) {
								...
								if (rowDelete.FILE_SEQ == null) {
									pstmt_tOracleOutput_2.setNull(1, java.sql.Types.VARCHAR);
								} else {
									pstmt_tOracleOutput_2.setString(1, rowDelete.FILE_SEQ);
								}

								if (rowDelete.FILE_TYP == null) {
									pstmt_tOracleOutput_2.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tOracleOutput_2.setString(2, rowDelete.FILE_TYP);
								}

								pstmt_tOracleOutput_2.setBigDecimal(3, rowDelete.FILE_SER);

								pstmt_tOracleOutput_2.setBigDecimal(4, rowDelete.FILE_NBR);

								try {
									deletedCount_tOracleOutput_2 = deletedCount_tOracleOutput_2
											+ pstmt_tOracleOutput_2.executeUpdate();
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "rowDelete");
				}
			...
			try {
				...
				if (resourceMap.get("statementClosed_tOracleOutput_2") == null) {
					...
					if ((pstmtToClose_tOracleOutput_2 = (java.sql.PreparedStatement) resourceMap
							.remove("pstmt_tOracleOutput_2")) != null) {
	...
	public static void main(String[] args) {
		final MARK_CONVERT MARK_CONVERTClass = new MARK_CONVERT();

		int exitCode = MARK_CONVERTClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = MARK_CONVERT.class.getClassLoader()
					.getResourceAsStream("wipo_projects/mark_convert_0_3/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = MARK_CONVERT.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : MARK_CONVERT");
		}
...
/************************************************************************************************
 * 123900 characters generated by Talend Open Studio for Data Integration on the
 * February 4, 2021 at 7:33:22 AM CET
 ************************************************************************************************/